...
public class DESStringTool extends JFrame {
    ...
    private void addEventHandlers() {
        ...
        // --- XỬ LÝ NÚT SINH KHÓA ---
        btnGenKeyEnc.addActionListener(e -> {
            try {
                // Sử dụng KeyGenerator để sinh khóa chuẩn cho DES (56 bits -> 8 bytes)
                ...
                // Cập nhật lên giao diện cho cả 2 bên
                txtEncryptKey.setText(hexKey.toString());
                txtDecryptKey.setText(hexKey.toString());
                
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Lỗi sinh khóa: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        });
